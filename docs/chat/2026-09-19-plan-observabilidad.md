...
**Sin iniciar sesión en Grafana ni en Sentry** (HECHO 2026-09-20, decisión de SDN: quien entra a `/entornos` con el PIN no tiene que autenticarse en esos servicios ni conocer la cuenta de GitHub de SDN). Se descartó el enlace público del tablero (cualquiera con la URL vería las métricas; el sistema lo bloqueó como debilitamiento de seguridad) y se trajo el contenido a la pestaña: (B) `observabilidad/metricas_panel.py` dibuja los gráficos y las tarjetas del tablero con el token de LECTURA de Grafana (proxy de la fuente Prometheus); las consultas se sacan de `aplicar_tablero.construir_tablero`, así no pueden diferir de las de Grafana; rangos 1 h/6 h/24 h/7 d, 45 s de caché. (C) `sentry_panel.detalle` muestra, por cada error, el tipo, el mensaje, los pasos de código de la app (archivo:línea, del más reciente al más viejo) y las etiquetas permitidas, sin abrir Sentry. Los enlaces a Grafana y Sentry siguen para quien quiera el detalle fino y tenga cuenta. Para dar acceso al equipo a esas herramientas: invitarlos con su propio usuario, nunca compartir credenciales.
