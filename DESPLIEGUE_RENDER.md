...
| `GRAFANA_METRICS_TOKEN` | — | Token de Grafana Cloud con permiso `metrics:write` y nada más | Solo Pruebas. Lo usa el hilo de la app que junta las métricas de Render y las escribe en Grafana (`observabilidad/hilo_colector.py`): es la segunda vía del colector, respaldo de GitHub Actions. Usa además `RENDER_API_KEY` (que ya está para la pestaña Planes). Sin alguna de las dos, o con `COLECTOR_METRICAS=off`, no arranca. |
| `SENTRY_DSN` | — | DSN del proyecto de Sentry (`colm3na-pruebas`) | Sin ella Sentry no hace nada. El DSN solo permite ENVIAR eventos, no leerlos; aun así va como variable y no en el repo. Ver `sentry_config.py`. |
| `TELEGRAM_BOT_TOKEN` | — | Token del bot `Colm3na_bot` (BotFather) | Avisos al teléfono (`telegram.py`): errores al instante, resumen diario, alertas de Grafana. Rotar en BotFather con Revoke y actualizar acá. |
| `TELEGRAM_CHAT_ID` | — | Id numérico del chat del equipo | Adónde van los avisos. No es secreto. Con `TELEGRAM_RESUMEN_HORA` (HH:MM, opcional, 21:00 por defecto) se cambia la hora del resumen. |
| `SENTRY_AUTH_TOKEN` | — | Token personal de Sentry, solo lectura (Project, Issue & Event y Organization: Read) | Lo usa la pestaña Observabilidad para MOSTRAR los errores (`observabilidad/sentry_panel.py`). Sin él se siguen enviando errores pero la pestaña dice "falta el token". No vence: revocarlo si se deja de usar. Reemplaza a la vieja `SENTRY_URL`, que ya no se lee. |
