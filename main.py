...
def admin(request: Request):
    ...
    with db.get_session() as s:
        ...
        trabajadores = s.exec(select(Trabajador).where(Trabajador.sindicato_id == sid)).all()
...
def admin_login(usuario: str = Form(...), clave: str = Form(...)):
    with db.get_session() as s:
        user = s.exec(select(UsuarioSindicato).where(
            UsuarioSindicato.usuario == usuario, UsuarioSindicato.activo == True)).first()
...
@app.post("/admin/cuils")
def admin_cargar_cuils(request: Request, cuils: str = Form(...)):
    """Carga la lista de CUILs habilitados del sindicato (uno por línea o separados por coma)."""
    sid = exigir_sindicato(request)
    import re
    crudos = re.split(r"[\s,;]+", cuils.strip())
    nuevos = 0
    with db.get_session() as s:
        ...
        for c in crudos:
            cuil = re.sub(r"[^0-9]", "", c)
            if len(cuil) == 11 and cuil not in existentes:
                s.add(Trabajador(sindicato_id=sid, cuil=cuil, registrado=False))
                existentes.add(cuil); nuevos += 1
        s.commit()
...
def plataforma_login(response: Response, clave: str = Form(...)):
    if not auth.verificar_plataforma(clave):
...
def plataforma_alta_sindicato(
    ...
    with db.get_session() as s:
        sind = Sindicato(
            nombre=nombre, descripcion=descripcion, slug=slugify(nombre),
...
def plataforma_alta_usuario(
    ...
    with db.get_session() as s:
        s.add(UsuarioSindicato(
            sindicato_id=sindicato_id, usuario=usuario, nombre=nombre,
...
# ================= Trabajador: registro, login y selección de sindicato =================
...
def app_trabajador(request: Request):
    ...
    # Un solo sindicato → entra directo
    if len(sinds) == 1:
        marca = db.marca_sindicato(sinds[0]["id"])
        return templates.TemplateResponse("trabajador.html", {
            "request": request, "sindicato": marca["nombre"], "marca": marca,
        })
    # Varios y ya eligió → entra a ese
    if elegido:
        for sd in sinds:
            if str(sd["id"]) == elegido:
                marca = db.marca_sindicato(sd["id"])
                return templates.TemplateResponse("trabajador.html", {
                    "request": request, "sindicato": marca["nombre"], "marca": marca,
                })
