...
def construir_md() -> str:
    ...
    a("")
    a("**Las conexiones no se pueden calcular, hay que medirlas.** Se multiplican por")
    a("instancia, pero las siete corridas muestran que el número sigue a la saturación del")
    a("servicio web y no a la cantidad de workers, y que se pasa del máximo que el pool")
    a("debería permitir (ver el hallazgo correspondiente). Hasta entender por qué, la única")
    a("forma seria de dimensionarlo es medirlo en la configuración real ya provisionada y")
    a("dejar holgura contra el límite del plan de Postgres elegido.")
    a("")
